...
trait QuestionOptionsGetPdfData
{
    ...
    public function QuestionOptionsGetPdfData($event): bool
    {
        ...
        if (!empty($data->results['resultImage'])) {
            $imageRatio = null;
            if (!empty($data->results['questionData']->imageSize) && !empty($data->results['questionData']->imageSize['height'])) {
                $imageRatio = $data->results['questionData']->imageSize['width'] / $data->results['questionData']->imageSize['height'];
            }

            $data->pdfSvg = [
                'svg' => $data->results['resultImage'],
                'imagePath' => $data->results['questionData']->imagePath,
                'imageRatio' => (float) $imageRatio,
            ];
        }

        $htmlResume = '<table><tr>';
        $htmlResume .= '<td>' . Text::_('PLG_QUIZTOOLS_HOTSPOTSMULTIPLE_PDF_RESUME_LABEL') . ': </td>';
        if ($data->is_correct) {
            $htmlResume .= '<td>' . Text::_('PLG_QUIZTOOLS_HOTSPOTSMULTIPLE_PDF_RESUME_CORRECT'). '</td>';
        } else {
            $htmlResume .= '<td>' . Text::_('PLG_QUIZTOOLS_HOTSPOTSMULTIPLE_PDF_RESUME_INCORRECT'). '</td>';
        }
        $htmlResume .= '</tr></table>';
        ...
        if ($data->withFeedback) {
            ...
            if (!empty($feedback['text'])) {
                $htmlFeedback .= '<p>' . Text::_('PLG_QUIZTOOLS_HOTSPOTSMULTIPLE_PDF_RESULT_FEEDBACK') . ': ' . $feedback['text'] . '</p>';
            }
