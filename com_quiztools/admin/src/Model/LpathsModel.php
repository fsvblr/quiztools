...
/**
 * Methods supporting a list of Learning Paths records.
 *
 * @since  1.6
 */
class LpathsModel extends ListModel
{
    /**
     * Constructor.
     *
    ...
    protected function populateState($ordering = 'a.id', $direction = 'desc')
    {
        ...
        $article_id = $this->getUserStateFromRequest($this->context . '.filter.article_id', 'filter_article_id');
	...
	protected function getListQuery()
	{
        ...
        // Filter by quiz / article (in lpath_items)
        $quiz_id = $this->getState('filter.quiz_id');
        if (is_numeric($quiz_id)) {
            $comparison = '"item_quiz_id":"' . (int) $quiz_id . '"';
            $query->where($db->qn('a.lpath_items') . ' LIKE ' . $db->q('%' . $comparison . '%'));
        }
        $article_id = $this->getState('filter.article_id');
        if (is_numeric($article_id)) {
            $comparison = '"item_article_id":"' . (int) $article_id . '"';
            $query->where($db->qn('a.lpath_items') . ' LIKE ' . $db->q('%' . $comparison . '%'));
        }
	...
	/**
	 * Getting a list of Learning Paths to build a 'select' in forms.
	 *
	 * @return array|mixed
	 * @throws \Exception
	 */
	public function getLearningPathsList()
	{
		$db = $this->getDatabase();
		$query = $db->createQuery()
