...
/**
 * This models supports retrieving lists of Learning Paths.
 *
 * @since  1.6
 */
class LpathsModel extends ListModel
{
    ...
    protected function populateState($ordering = 'a.ordering', $direction = 'ASC')
    {
	    ...
	    // Page for a specific Learning Path category. There is no filter by category.
	    $category_id = $input->getInt('catid');
		if (!$category_id) {
			// Page for all Learning Paths of all categories. There is a filter by categories.
    ...
    /**
     * Method to get a list of Learning Paths.
     *
     *
    ...
    public function getItems()
    {
		...
		if (!empty($items)) {
			foreach ($items as $item) {
				...
				// will be shown in the category. Otherwise, there is no Learning Path description in the category.
