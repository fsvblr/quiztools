...
trait QuestionOptionsGetResults
{
    ...
    public function QuestionOptionsGetResults($event): bool
    {
        ...
        $query->clear();
        $query->select($db->qn(['blank_id', 'answer', 'is_correct']))
        ...
        foreach ($options as $option) {
            ...
            if (!empty($results)) {
                foreach ($results as $result) {
                    if ((int) $option->id === (int) $result->blank_id) {
                        $option->is_correct = $result->is_correct;
                        $option->user_answer = $result->answer;
                        break;
                    }
                }
            }
        ...
        if (!empty($data->results)) {
            ...
            foreach ($data->results as $i => $result) {
                ...
                $article->text = $result->user_answer;
                ...
                if (!empty($article->text)) {
                    $data->results[$i]->user_answer = $article->text;
                }
                unset($article);
            }
        }
