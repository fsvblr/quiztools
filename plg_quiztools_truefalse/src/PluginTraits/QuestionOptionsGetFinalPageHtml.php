    ...
    public function QuestionOptionsGetFinalPageHtml($event): bool
    {
        ...
        $html .= '<div class="result-options ' . $this->name . '">
                <div class="result-options-header">
                    ...
                    <div class="w-65">' . Text::_('PLG_QUIZTOOLS_TRUEFALSE_FINAL_RESULT_HEADING_OPTION_TEXT') . '</div>
                    <div class="w-15 text-center">' . Text::_('PLG_QUIZTOOLS_TRUEFALSE_FINAL_RESULT_HEADING_OPTION_RIGHT_ANSWER') . '</div>
                    <div class="w-15 text-center">' . Text::_('PLG_QUIZTOOLS_TRUEFALSE_FINAL_RESULT_HEADING_OPTION_USER_ANSWER') . '</div>
                </div>';
        ...
        $html .= Text::_('PLG_QUIZTOOLS_TRUEFALSE_FINAL_RESULT_USER_SCORE') . ': ';
        $html .= (float) $data->points_received . '/' . (float) $data->total_points;
        $html .= '</div>';

        // Feedback logic, if needed, is similar to mchoice but simplified
        if ($data->withFeedback) {
            ...
            if (!empty($feedback['text'])) {
                $html .= '<div class="result-options-feedback"><span>' .
                    Text::_('PLG_QUIZTOOLS_TRUEFALSE_FINAL_RESULT_FEEDBACK') . ': </span>' . QuiztoolsHelper::cleanHtml($feedback['text']) . '</div>';
            }
        }
